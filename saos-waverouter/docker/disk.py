...
class CommandFailed(Exception):
    def __init__(self, returncode, cmd, output, stderr):
        ...
        self.stdout = output
        self.stderr = stderr
        super().__init__(f"Command '{cmd}' failed with return code: {returncode}, stderr: {stderr}")
...
def run_command(cmd, cwd=None, background=False, shell=False):
    ...
    # This is a workaround to check for stderr to raise exception, p.returncode returns 0 even when command failed. Need to investigate!
    if stderr:
        logger.error(f"STDERR: {stderr}")
        raise CommandFailed(returncode=p.returncode, cmd=cmd, output=stdout, stderr=stderr)
    return stdout
...
def losetup(disk_file):
    ...
    try:
        ...
        stdout = run_command(losetup_cmd)
        run_partition_helper()
        loop_device = stdout.decode().rstrip("\n\r")
...
def create_disk_image(path, size, partitions, guest_name):
    try:
        ...
        logger.error(e)
        try:
            logger.info(f"Removing disk image {path}\n")
            os.remove(path)
        except FileNotFoundError:
            pass
        return False
